...
const url = "http://backend:3000/api/v1/people";
...
const PersonList = () => {
  ...
  const deletePerson = async(id) => {
    try {
      await axios.delete(`http://backend:3000/api/v1/people/${id}`);
  ...
  const enterUpdateState = async (id) => {
    try{
      const response = await axios(`http://backend:3000/api/v1/people/${id}`);
